...
class SomosClient {
  ...
  async getArtists(value) {
    const response = await fetch(`https://api.spotify.com/v1/search?q=${value}&type=artist`, {
      method: 'GET',
  ...
  async showArtist(id) {
    ...
    const responseAlbums = await fetch(`https://api.spotify.com/v1/artists/${id}/albums?limit=10`, {
      method: 'GET',
      headers: this.headers,
    })
