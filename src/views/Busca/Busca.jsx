...
class Busca extends React.Component {
  constructor(props) {
    ...
    this.timeout = 0
    const artistis = this.verifyOldSearch()
    this.state = {
      artists: artistis,
          ...
          <div className={styles.wrapper}>
            <div className={styles.searchInput}>
              <TextBox
                ...
                onChange={value => this.searchEvent(value, 1000)}
                placeholder={translate('busca.buscar_artista')}
              />
              {bLoading ? (
              ...
              {bNoResults ? (
                <div className={styles.noResults}>
                  {translate('busca.nenhum_resultado_encontrado')}
                </div>
              ) : (
                ''
              )}
            </div>
            <div className={styles.listArtists}>{lstArtists}</div>
          </div>
        </React.Fragment>
      </Layout>
    )
