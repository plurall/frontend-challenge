import React from 'react'

import { Layout } from 'components'
import { SubHeader } from 'components'
import { SomosClient } from 'utils'

import styles from './Artist.module.css'

class Artist extends React.Component {
  constructor(props) {
    super(props);
    this.state = {artist: {}, albums: {}};
    this.client = new SomosClient();
  }

  componentDidMount () {
    const { id } = this.props.match.params
    this.getAlbums(id);
    this.getArtist(id);
  }

  async getAlbums(id) {
    this.setState({albums: await this.client.getArtistAlbums(id)});
  }

  async getArtist(id) {
    this.setState({artist: await this.client.getArtist(id)});
  }

  render() {
    let artist = null;
    let genres = [];

    if (this.state.artist !== null) {
      if (this.state.artist.genres) {
        this.state.artist.genres.map((genre) => {
          genres.push(<span className={styles.genre}>{genre}</span>);
        });
      }

      artist = <div className={styles.artist}>
        <h2>{this.state.artist.name} <small>Popularity: {this.state.artist.popularity}</small></h2>
        <p>{genres}</p>
      </div>;
    }

    let albums = [];
    if (this.state.albums.items) {
      this.state.albums.items.map((row) => {

        let image = null;
        if (row.images[0]) {
          image = <img src={row.images[0].url} width="120px" height="120px"/>;
        } else {
          image = <img src="data:image/png;base64," width="120px" height="120px"/>;
        }

        albums.push(
          <div className={styles.album}>
            <div><p>{image}</p></div>
            <div className={styles.description}>
              <p><strong>{row.name}</strong></p>
              <p>{new Date(row.release_date).toLocaleDateString()}</p>
              <p className={styles.tracks}>{row.total_tracks} tracks</p>
            </div>
          </div>
        )
      });
    }

    return (
      <Layout>
        <SubHeader
          breadcrumb={[{ text: 'Artist' }]}
          heading="Somos Front-end Challange"
        />
        <div className={styles.wrapper}>
          {artist}
          <h4>Albums</h4>
          <div className={styles.albums}>
            {albums}
          </div>
        </div>
      </Layout>
    )
  }
}

export default Artist
