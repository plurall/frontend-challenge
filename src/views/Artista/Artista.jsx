...
class Artista extends React.Component {
  ...
  constructor(props) {
    ...
    if (this.props.location.artist) {
      ...
      sessionStorage.setItem(
        'artists',
        JSON.stringify(this.props.location.artists),
      )
      sessionStorage.setItem(
        'searchInputValue',
        this.props.location.searchInputValue,
      )
    } else {
      this.searchArtist()
    }
  ...
  getArtistAlbuns = () => {
    this.client
      .getArtistAlbuns(this.props.match.params.id)
      .then(resolve => {
        if (resolve.error) {
          console.log(resolve.error)
        } else {
      ...
      <Layout>
        <React.Fragment>
          <SubHeader
            breadcrumb={[
              { text: 'Home', href: '/' },
              { text: translate('busca.busca'), href: '/busca' },
              { text: translate('artista.artista') },
            ]}
          ...
          <div className={styles.wrapper}>
            ...
            <div className={styles.listAlbuns}>
              {bLoadingAlbuns ? (
                <Loader text={translate('artista.carregando_albuns')} />
              ) : (
                ''
              )}
              {lstAlbuns}
